// Group Query Log Ignored View
// Shows merged ignored domains for query logs in a group (read-only)

import { escapeHtml } from '../utils.js';
import { deduplicateIgnoredDomains } from '../utils.js';

export async function renderGroupQueryLogIgnored(container, data = {}) {
    const { groupId } = data;
    ...
    if (group.serverIds && group.serverIds.length > 0) {
        try {
            ...
            for (const serverId of group.serverIds) {
                ...
                if (server) {
                    try {
                        const config = await window.app.sendMessage('getQueryLogConfig', { server });
                        if (config && config.ignored) {
                            allDomains.push(...config.ignored);
                        }
                    } catch (e) {
                        console.debug(`Failed to fetch query log config from server ${serverId}:`, e);
                    }
                }
            }
    ...
    container.innerHTML = `
        <div class="view-container">
            <div class="view-header">
                ...
                <h1 class="view-title">QUERY LOG IGNORED</h1>
                ...
                <div style="background: var(--color-bg-secondary); border: 1px solid var(--color-border); border-radius: 6px; padding: 12px; margin-bottom: 14px;">
                    ...
                    <div>
                        ...
                        <div style="display: flex; gap: 8px;">
                            <div style="background: var(--color-bg-primary); border: 1px solid var(--color-border); border-left: 2px solid #42a5f5; border-radius: 4px; padding: 6px;">
                                <div style="font-size: 16px; font-weight: 700; color: #42a5f5; line-height: 1;">${mergedDomains.length}</div>
                    ...
                    <div id="domains-container" style="max-height: 239px; overflow-y: auto; display: flex; flex-direction: column; gap: 6px;">
                        ${mergedDomains.length === 0 ? `
                            <div style="text-align: center; padding: 30px; color: var(--color-text-secondary);">
                                ...
                                <div style="font-size: 10px; opacity: 0.7;">Configure ignored domains in query log settings</div>
                            </div>
                        ` : mergedDomains.map(domain => `
                            <div style="background: rgba(255, 255, 255, 0.02); border-radius: 6px; padding: 12px; border-left: 3px solid #42a5f5; transition: all 0.2s ease;">
